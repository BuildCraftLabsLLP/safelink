...
**Favicon / app icon**: `static/favicon.svg` is a ~255-byte vector "SL" mark (white on `#FF9933`) linked from `<head>` in `templates/base.html` (and `static/alerts.html`) via `<link rel="icon" type="image/svg+xml">` — chosen as an SVG so it stays tiny and never competes with the per-page HTML budget. It is paired with `static/favicon.ico` (a 2 KB multi-res 16/32/48 raster, generated from `icon-512.png` via Pillow) declared first with `sizes="any"` — this is the fallback for browsers that ignore SVG favicons and it answers the automatic `/favicon.ico` request (without it that path hits the SPA fallback and returns HTML). The same head links `apple-touch-icon` to `static/icon-192.png` for the iOS home screen; Android/PWA install icons come from `icon-192.png`/`icon-512.png` in `manifest.json`. If the "SL"/orange branding changes, regenerate `favicon.svg`, `favicon.ico`, the two PNG icons, and update `theme_color`/`background_color` in `manifest.json` together. Note: browsers (esp. Chrome) cache favicons aggressively — a changed icon may need a hard cache clear or incognito to appear.
